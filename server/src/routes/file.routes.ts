...
import {
  ...
  uploadMultiple,
  uploadChunk,
} from "@/middlewares/upload/upload.middleware";
import {
  uploadRateLimit,
  creationRateLimit,
...
router.post("/upload/chunk/complete", (req, res, next) =>
  getController().completeChunkUpload(req, res, next),
);
...
router.post("/upload/cancel", validate(cancelUploadSchema), (req, res, next) =>
  getController().cancelUpload(req, res, next),
);

// File Management Routes
router.get("/", (req, res, next) => getController().getFiles(req, res, next));
router.get("/:id", (req, res, next) =>
  getController().getFileById(req, res, next),
);
router.get("/:id/download", (req, res, next) =>
  getController().downloadFile(req, res, next),
);
router.get("/:id/preview", (req, res, next) =>
  getController().getPreview(req, res, next),
);
router.get("/:id/thumbnail", (req, res, next) =>
  getController().getThumbnail(req, res, next),
);
...
router.delete("/:id", (req, res, next) =>
  getController().deleteFile(req, res, next),
);
...
router.post("/:id/favorite", (req, res, next) =>
  getController().favoriteFile(req, res, next),
);
router.post("/:id/unfavorite", (req, res, next) =>
  getController().unfavoriteFile(req, res, next),
);
...
router.delete("/:id/share", (req, res, next) =>
  getController().deleteShare(req, res, next),
);
